...
import validation_1_hand_pi06_imgN as single_eval
...
def _resolve_episode_dirs(args: Args) -> list[str]:
    ...
    if args.episode_glob:
        episode_dirs.extend(path for path in glob.glob(args.episode_glob) if pathlib.Path(path).is_dir())
    if not episode_dirs:
...
def eval_isaac_multi(args: Args) -> None:
    ...
    print(f"Resolved {len(episode_dirs)} episode(s) for PI06 visualization.")
    for episode_idx, episode_dir in enumerate(episode_dirs, start=1):
        ...
        if prompt_types:
            ...
            episode_args = dataclasses.replace(args, prompt_type=prompt_type, prompt=single_eval.prompt_map[prompt_type])
        single_eval._eval_single_episode(client, episode_args, episode_dir)


if __name__ == "__main__":
    eval_isaac_multi(tyro.cli(Args))
