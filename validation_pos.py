...
episode_dir = "/b3-mix03/sppro/permanent/yuanzhang10/资源部传输数据/叠衣服/SL-叠衣服-全检结束-v2/SL-短袖0930_全检结束/0930_05_短袖_SL_1件/episode_20250930155249"
...
def eval_isaac(args: Args) -> None:
    ...
    states_joint = df[[f"l_s_{i}" for i in range(7)] + [f"r_s_{i}" for i in range(7)]].values.astype(np.float32)
    ...
    for episode_idx in tqdm.tqdm(range(args.num_trials_per_task)):
        ...
        for i in range(states_joint.shape[0]):
            try:
                img = head_frames[i]
                ...
                states_l = fk.set_joint_states_get_pose(states_joint[i][:6],base_tip_link_index=0)   # -> [x,y,z,qw,qx,qy,qz]
                states_r = fk.set_joint_states_get_pose(states_joint[i][7:13],base_tip_link_index=0)
                ...
                states = states_l + [actions_joint[i][6]] + states_r + [actions_joint[i][6]]
                actions = actions_l + [actions_joint[i][6]] + actions_r + [actions_joint[i][6]]
                ...
                if not action_plan:
                    ...
                    element = {
                        ...
                        "observation/joint_position": np.array(states),
                ...
                action_error = np.mean(np.abs(action[:16] - np.array(actions[:16])))
                ...
                label1.append(actions[0])
                label2.append(actions[1])
                label3.append(actions[2])
                label4.append(actions[3])
                label5.append(actions[4])
                label6.append(actions[5])
                label7.append(actions[6])
                label8.append(actions[7])
                label9.append(actions[8])
                label10.append(actions[9])
                label11.append(actions[10])
                label12.append(actions[11])
                label13.append(actions[12])
                label14.append(actions[13])
                label15.append(actions[14])
                label16.append(actions[15])
        ...
        plt.tight_layout()
        plt.savefig('/work2/cv5/jfni3/openpi06/val.png')
        # plt.show()
