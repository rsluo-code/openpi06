...
def train_loop(config: _config.TrainConfig):
    ...
    global_step = 0
    if resuming==False and config.pytorch_weight_path is not None:
        logging.info(f"Loading weights from: {config.pytorch_weight_path}")
        load_param_from_ckpt_dir(model,config.pytorch_weight_path,"PI06")
        logging.info(f"Loaded PyTorch weights from {config.pytorch_weight_path}")
    elif resuming==False and config.pytorch_weight_path is  None:
        raise RuntimeError("尚未配置config.pytorch_weight_path")
    else:
        ...
        logging.info(f"Loading model state from {ckpt_dir}")
        load_param_from_ckpt_dir(model,ckpt_dir,"PI06")
    ...
    print(f"从global_step={global_step}开始")
