...
def train_loop(config: _config.TrainConfig):
    ...
    while global_step < config.num_train_steps:
        ...
        for observation, actions, step_index, episode_length,language_instruction_index,language_instruction_max_len,success_or_failure in loader:
            ...
            # bin_values = torch.linspace(v_min, v_max, num_bins, device=device)  # [-1,0] 对应 201 个值
            # pred_value = (probs * bin_values).sum(dim=-1)  # [B]
            # MSE_LOSS_SCALED = 10
            # loss = torch.nn.functional.mse_loss(pred_value, Rt)*MSE_LOSS_SCALED
