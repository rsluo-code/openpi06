...
def cleanup_robot(robot_id, physics_client):
    ...
    幂等清理：
    - 先判断 client 是否连接；
    - 若连接，先尝试 removeBody，再 disconnect；
    - 全程带 clientId，且吞掉析构期的异常。
    """
    try:
        # 规范化输入
        cid = physics_client if isinstance(physics_client, int) else -1
        rid = robot_id if isinstance(robot_id, int) else -1

        # 判断是否仍然连接
        connected = False
        try:
            if cid >= 0:
                connected = bool(p.isConnected(physicsClientId=cid))
            else:
                connected = bool(p.isConnected())
        except Exception:
            connected = False
