...
import openpi.models_pytorch.pi06_pytorch
...
def train_loop(config: _config.TrainConfig):
    ...
    model = openpi.models_pytorch.pi06_pytorch.PI06Pytorch(model_cfg)
    ...
    def load_checkpioint_form_ckpt_dir(model, ckpt_dir: str):
        ckpt_path = os.path.join(ckpt_dir, "model.safetensors")
        print(f"[pi06] Loading Pi05  weights from: {ckpt_path}")
        # 1. 读 ckpt
        ...
        # for key in tgt_sd.keys():
            # print(f"pi06 tgt_sd:{key}")
        ...
        print(f"[pi06] Will load {len(filtered_sd)} parameters into paligemma_with_expert.paligemma")
        ...
        print(f"[pi06] load_state_dict done. missing={len(missing)}, unexpected={len(unexpected)}")
        if missing:
            print(f"[pi06] missing keys (正常的通常只包括 value_head.*): {missing}")
        if unexpected:
            print(f"[pi06] unexpected keys from ckpt (已忽略): {unexpected}")
    ...
    if config.pytorch_weight_path is not None:
        ...
        # )
        load_checkpioint_form_ckpt_dir(model,config.pytorch_weight_path)
