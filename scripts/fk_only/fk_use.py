...
class FKCSVPlayer(Node):
    def __init__(self, config_path: str, csv_path: str, fk_name: str = "fk1", base_tip_index: int = 0, loop: bool = False):
        ...
        self.base_tip_index = base_tip_index
        self.loop = loop
    ...
    def run_once(self):
        with open(self.csv_path, "r", newline="") as f:
            ...
            for i, row in enumerate(rows):
                ...
                # FK计算
                try:
                    pose = self.fk.set_joint_states_get_pose(cur_joint, base_tip_link_index=self.base_tip_index)
                ...
                # 发布TF
                trans, quat_xyzw = _as_quaternion_xyzw_from_fk_pose(pose)
                t = TransformStamped()
                t.header.stamp = self.get_clock().now().to_msg()
                t.header.frame_id = BASE_FRAME
                t.child_frame_id = END_FRAME
                ...
                js.name = JOINT_NAMES
                js.position = cur_joint
                self.js_pub.publish(js)

                # 控制速率
                if have_all_ts:
                    target_wall = start_wall + max(0.0, ts_list[i] - csv_t0)
...
def main():
    ...
    parser.add_argument("--fk-name", default="fk1", help="DualArmFK 名称，默认fk1")
    parser.add_argument("--base-tip-index", type=int, default=0, help="FK base_tip_link_index，默认0")
    parser.add_argument("--loop", action="store_true", help="是否循环播放CSV")

    args = parser.parse_args()

    rclpy.init()
    node = FKCSVPlayer(args.config, args.csv, args.fk_name, args.base_tip_index, args.loop)
...
"""
python3 /home/rsluo/project/about_tele/iflytek_armtele_project/src/fk_only/fk_use.py  --config /home/rsluo/project/about_tele/iflytek_armtele_project/src/fk_only/ConfigTracikTeleop.yaml --csv /home/rsluo/下载/0919-1031/robot.csv
