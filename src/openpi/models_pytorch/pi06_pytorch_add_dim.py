...
class PI06Pytorch(nn.Module):
    def __init__(self, config):
        ...
        # self.success_token_embed = nn.Linear(1, paligemma_config.width)
        ...
        if config.pi06_2_pi05 == False:
            ...
            self.pi06_2_pi05 = False
        else:
            self.pi06_2_pi05 = True

        self.sample_actions = torch.compile(self.sample_actions, mode="max-autotune")
        print(f"model training = {self.training}")
        ####
        if self.config.if_use_valuenet:
            ...
            load_param_from_ckpt_dir(self.value_net,
            "/wx-mix01/sppro/permanent/yuanzhang10/codes_rsluo/openpi06/pi06_torch/value_pretrain_16dim/valuenet_pretrain_20260116_2b_16dim_exchangimg/360000",
            "pi06_value_net")
            # "/wx-mix01/sppro/permanent/yuanzhang10/codes_rsluo/openpi06/pi06_torch/value_pretrain_16dim/valuenet_train_20260106_300m_16dim_exchangsize/55000",
    ...
    def embed_prefix(
        ...
        if self.pi06_2_pi05 ==False:
#------------------------------------------------------------------
            # ====== 新增：indicator 条件 token ======
            # 如果没有提供 indicator，则默认全部为 1（即模型推理时默认选择成功）
            # indicator=None
            if indicator is None:
                indicator = self.indicator_pool[:bsize].to(lang_emb.device)
...
#------------------------------------------------------------------
    ...
    # def forward(self, observation, actions, noise=None, time=None) -> Tensor:
        with torch.no_grad():
            if self.training and  self.pi06_2_pi05 ==False:
        ...
        for batch_idx in range(batch_size):
            ...
            for step_idx in range(num_steps):
                ...
                if actions[batch_idx,step_idx,15] <=0.00019904:  # (x - q01) / (q99 - q01 + 1e-6) * 2.0 - 1.0    # or actions[batch_...
                    gaussian_centers.append(step_idx)
    ...
    def sample_actions(self, device, observation, noise=None, num_steps=10) -> Tensor:
        ...
        # print(f"sample_actions model training = {self.training}")
