...
class ValueNetPytorch(nn.Module):
    ...
    def __init__(self, config, num_bins: int = 201, image_keys: Sequence[str] | None = None):
        ...
        self.register_buffer("bin_centers", bin_centers)    # 关键：注册为 buffer
        ...
        print(f"paligemma_config:{paligemma_config}")
        # 对比 pi0_pytorch：
    ...
    def embed_prefix(
        ...
        for img, img_mask in zip(images, img_masks, strict=True):
            ...
            def image_embed_func(x):
                # print(f"image_embed_func:x.shape={x.shape}")
        ...
        # # 在valuenet_pytorch.py第268行（torch.cat之前）添加以下代码
        # print("=== 调试：embs列表中每个张量的形状 ===")
        # for i, emb in enumerate(embs):
        #     print(f"张量{i}形状：{emb.shape}")
