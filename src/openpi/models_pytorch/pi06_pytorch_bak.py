...
class PI06Pytorch(nn.Module):
    def __init__(self, config):
        ...
        self.success_token_embed = nn.Embedding(2, paligemma_config.width)
        ...
        valuenet_model_cfg = openpi.models.pi0_config.Pi0Config(
            action_horizon=30, 
            paligemma_variant="gemma_300m_valuenet", 
        ...
        load_checkpioint_for_value_net(self.value_net,"/wx-mix01/sppro/permanent/yuanzhang10/codes_rsluo/openpi06/pi06_torch/value_pretrain_16dim/valuenet_train_20260106_300m_16dim_exchangsize/55000")
        # 冻结 value_net 的所有参数
    ...
    def embed_prefix(
        self, images, img_masks, lang_tokens, lang_masks,indicator=None,   # (B,) 取值 0/1，torch.long
...
#------------------------------------------------------------------
        # ====== 新增：indicator 条件 token ======
        # 如果没有提供 indicator，则默认全部为 1（即模型推理时默认选择成功）
        if indicator is None:
            indicator = torch.ones(bsize, dtype=torch.long, device=lang_emb.device)
        
        # (B,) -> (B, D) -> (B, 1, D)   假设 D 和 lang_emb 一致
        cond_emb = self.success_token_embed(indicator.squeeze(-1))[:, None, :].to(dtype=lang_emb.dtype)

        embs.append(cond_emb)

        # mask: 这个 token 永远有效
        cond_mask = torch.ones(bsize, 1, dtype=torch.bool, device=lang_emb.device)
        pad_masks.append(cond_mask)

        # attention: prefix 内部全互相可见，所以这里是 0
        att_masks += [0]
#------------------------------------------------------------------
    ...
    # def forward(self, observation, actions, step_index, episode_length,language_instruction_index,language_instruction_max_len,success_or_failure ,language_instruction_at_30precent, noise=None, time=None) -> Tensor:
    def forward(self, observation,observation_tN, actions, step_index, episode_length,language_instruction_index,language_instruction_max_len,success_or_failure ,language_instruction_at_30precent, noise=None, time=None) -> Tensor:
        print("start forward")
        # with torch.no_grad():
        #     indicator = self.value_net.forward_cal_indicator( observation,observation_tN, step_index, episode_length, language_instruction_max_len,language_instruction_at_30precent)
        ...
        for batch_idx in range(batch_size):
            ...
            for step_idx in range(num_steps):
                ...
                if actions[batch_idx,step_idx,7] <=0.00019904:  # (x - q01) / (q99 - q01 + 1e-6) * 2.0 - 1.0    # or actions[batch_...
        ...
        final_weight = weights + special_weights
        ...
        weighted_loss = weighted_loss * (success_weight * indicator + failure_weight * (1 - indicator))
