...
##########
        self.success_token_linear = nn.Linear(1, paligemma_config.width)
        self.success_token_linear = self.success_token_linear.to(dtype=torch.bfloat16)

        # with torch.no_grad():
        #     self.success_token_linear.weight.zero_()
        #     if self.success_token_linear.bias is not None:
        #         self.success_token_linear.bias.zero_()
        self.max_bsize = 1024
        # 提前初始化内存池张量
...
##########
        ...
        if config.if_use_valuenet == True:
            ...
            # valuenet_checkpoint_dir = "/wx-mix01/sppro/permanent/yuanzhang10/codes_rsluo/openpi06/pi06_torch/value_pretrain_16dim/valuenet_pretrain_20260203_2b_8dim_noexchangimg_only5item_valbase90maxlen/100000"
    ...
    def embed_prefix(
        ...
        # ====== indicator 条件 token（Linear版）======
        if indicator is None:
            bsize = images.shape[0]
            indicator = self.indicator_pool[:bsize].to(images.device)

        # indicator: [B, 1] or [B] 都行，统一成 [B,1,1]
        ind = indicator
        if ind.ndim == 2 and ind.shape[1] == 1:
            ind = ind.squeeze(1)          # [B]
        ind = ind.to(device=lang_emb.device)

        # 用 float 做线性层输入：0/1 -> [B,1] -> [B,1,1]
        ind_f = ind.to(dtype=lang_emb.dtype).unsqueeze(1).unsqueeze(2)  # [B,1,1]

        # 线性投影到 D 维：输出 [B,1,D]
        cond_emb = self.success_token_linear(ind_f)  # [B,1,D]
    ...
    def forward(self, observation,observation_tN, actions, step_index, episode_length,language_instruction_index,language_instruction_max_len,success_or_failure ,language_instruction_at_30precent, noise=None, time=None) -> Tensor:
        ...
        # 108, 4022, 235292, 235248
        # 105985, 235292, 6222, 235289, 108, 4022, 235292, 235248
        # 105985, 235292, 8322, 235289, 108, 4022, 235292, 235248
