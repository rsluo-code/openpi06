...
NAME_SAVE = f"valuenet_20260420_8wstep_8dim_noechangemodel_valbase_700maxlen_{LEIBIE}"
...
class Args:
    ...
    # Plot output
    out_png: str = f"{PATH_SAVE_BASE}/{NAME_SAVE}_val.png"
    out_png_At: str = f"{PATH_SAVE_BASE}/{NAME_SAVE}_At.png"
    out_png_It: str = f"{PATH_SAVE_BASE}/{NAME_SAVE}_It.png"
...
def eval_isaac(args: Args) -> None:
    ...
    elif USE_LEFT and USE_RIGHT==False :
        left_video_path = episode_dir + "/cam_left/cam_left.mp4"
        right_video_path = episode_dir + "/cam_left/cam_left.mp4"
    elif USE_LEFT==False and USE_RIGHT :
    ...
    At1 = [b - a for a, b in zip(Rt1_list, Rt0_list)]
    At1 = At1[:-chunk_size]
    max_len = max_len
    max_len = L
    At2 = [Rt1_list[i + chunk_size] - Rt1_list[i] + (-chunk_size/max_len) for i in range(len(Rt1_list) - chunk_size)]
    plt.figure(figsize=(12, 4))
    plt.plot(timesteps_cut, At1, linewidth=1.0, label="At1 = Rt0_list - Rt1_list")
    plt.plot(timesteps_cut, At2, linewidth=1.0, label="At2 = RtN + Rt1_list[i+n] - Rt1_list[i] ")
    plt.title("ValueNet: At1  vs  At2")
    plt.xlabel("Timestep (step_index)")
    plt.ylabel("R(t) in [-1, 0]")
    plt.legend()
    plt.tight_layout()
    plt.savefig(args.out_png_At, dpi=200)
    print(f"Saved plot to: {args.out_png_At}")
    It1 = [a > at_30precent for a in At1]
    It2 = [a > at_30precent for a in At2]
    plt.figure(figsize=(12, 4))
    plt.plot(timesteps_cut, It1, linewidth=1.0, label="It1 = At1>At1_30precent")
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(args.out_png_It, dpi=200)
    print(f"Saved plot to: {args.out_png_It}")
